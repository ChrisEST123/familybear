...
const CommandTestScreen: React.FC = () => {
    ...
    return (
        <View style={styles.container}>
            <Button
                title="🔊 Play Heartbeat1"
                onPress={() => playSoundPattern('heartbeat1')}
            />
            <Button
                title="📢 Play Alert Sound"
                onPress={() => playSoundPattern('alert')}
            />
            <Button
                title="🔥 Set Temperature to 36.5"
                onPress={() => setTemperature(36.5)}
            />
            ...
            <Button
                title="📡 Read Last Played Sound"
                onPress={readSoundStatus}
            />

            {fsr && (
                <Text>
                    FSR: pickedUp={fsr.pickedUp ? 'Yes' : 'No'}, value=
                    {fsr.value}
                </Text>
            )}
            {gps && (
